...
def check_canonicity_many():
    output_file = "canonicity_survey_test_1.txt"

    raw_file = "canonicity_raw_test_1.txt"

    token_counts = [1, 50, 100, 200, 300, 500]
    step_counts = [100, 100, 100, 100, 100, 100]

    groups = 2 # change to 6
    batches_per_group = 3 # change to 100
    
    # for i in range(groups): 
    #     token_count = token_counts[i]
    #     steps = step_counts[i]

    #     actual_tokens = sample_tokens(batches_per_group, token_count, steps) # change to 100
    #     text = tokenizer.batch_decode(actual_tokens)
    #     canonical_tokens = tokenizer.batch_encode(text)

    #     canon_count = 0

    #     for j in range(batches_per_group): # change to 100
    #         actual_batch = actual_tokens[j]
    #         canonical_batch = canonical_tokens[j]
    #         canon = check_canonicity_bool(actual_batch, canonical_batch)
    ...
    #             file.write("Canonical? " + ("Yes" if canon else "no"))
    ...
    #     percent = (canon_count / (groups * batches_per_group)) * 100
    ...
    for i in range(2): 
        ...
        for j in range(3):
            ...
            if canon:
                canon_count += 1
        percent_canonicity = canon_count
